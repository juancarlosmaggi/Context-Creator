...
def test_gitignore(path: str):
    ...
    result = {
        "path": path,
        "is_ignored": should_ignore(full_path, base_path_str, git_root_str if git_root else None, ignore_spec, context_ignore_spec, name=full_path.name, base_path_prefix=base_path_prefix, git_root_prefix=git_root_prefix),
...
async def process_files_route(selected_paths: List[str] = Form(...)):
    ...
    base_path = Path.cwd()
    return StreamingResponse(process_files(selected_paths, base_path), media_type="text/plain")
